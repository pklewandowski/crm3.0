...
class DocumentForm(RestForm):
    def __init__(self, document_type, instance):
        super(DocumentForm, self).__init__()
        AttributeUtils().get_attributes(parent=None,
                                        document_type=document_type,
                                        level=self.form_attributes,
                                        type='FORM',
                                        status=instance.status if instance else doc_utils.get_initial_status(type=document_type))
            ...
            for i in node:
                ...
                if i['code'] == 'annex':
                    if instance:
                        ...
                        lov.extend(
                            [{
                                "lov_label": '{code} z dn.: {day}, wartość: {value}'.format(code=j.code, day=j.product.start_date, value='{:,.2f}'.format(j.product.value).replace(',', ' ').replace('.', ',')),
                                "lov_value": j.pk,
                                "lov_description": ''
