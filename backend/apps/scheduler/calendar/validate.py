...
class Validate:
    ...
    def validate_permissions(user, schedule=None, initial=True):
        if initial:
            if schedule:
                if not (user.has_perm('schedule.change_all_schedule') or user.has_perm('schedule.change_schedule')):
                    raise Exception('Użytkownik nie ma uprawnień do edycji zdarzenia')

                if not user.has_perm('schedule.change_all_schedule') \
                        and schedule.host_user.pk != user.pk \
                        and schedule.created_by.pk != user.pk:
                    raise Exception('Użytkownik nie ma uprawnień do edycji zdarzeń innych użytkowników')

            else:
                if not (user.has_perm('schedule.add_all_schedule') or user.has_perm('schedule.add_schedule')):
                    raise Exception('Użytkownik nie ma uprawnień do dodawania zdarzenia')
        else:
            if not user.has_perm('schedule.add_all_schedule') and schedule.host_user.pk != user.pk:
