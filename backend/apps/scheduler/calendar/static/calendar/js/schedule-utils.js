...
//--TIME-SLIDER HANDLER
// let dateSlider = document.getElementById('time-slider');
// let initialStartMinute = 0,
//     initialEndMinute = 1440,
//     step = 15,
//     margin = 15;

// let slider=noUiSlider.create(dateSlider, {
...
// });
// $('#time-slider-container .noUi-value.noUi-value-horizontal.noUi-value-large').each(function () {
//
//     let val = $(this).html();
...
function fillEventData(event_data, event) {
    ...
    $('form#schedule-form #id_schedule-type option').not(':selected').remove();
    ...
    if (ms) {
        $.each(ms, function (i, e) {
            ...
            let msContainer = $('#event-message-container').find('ul#event_messages');
...
function setInitialData(start, end) {
    ...
    $('form#schedule-form input#id_' + g_form_prefix + '-start_date').data("DateTimePicker").date(start.utc());
    $('form#schedule-form input#id_' + g_form_prefix + '-end_date').data("DateTimePicker").date(end.utc());

    $('#id_schedule-end_date').data("DateTimePicker").minDate(start.add(15, 'minutes').utc());
...
function eventDataDecode(ev) {
    let guide = ['id', 'title', 'start', 'end', 'color', 'viewable', 'editable', 'details_viewable', 'data'];
    let dataGuide = ['single_person', 'whole_day', 'confirmed', 'host_user', 'address', 'resizable', 'status', 'invited_users'];
    let userGuide = ['pk', 'first_name', 'last_name', 'email', 'phone_one', '_edit_link'];
...
function getEvents(start, end, callback) {
    ...
    let data = {
        ...
        filter_phrase: $('#scheduleFilterModal').find('#event_filter_phrase').val(),
...
function getFormContent(mode) {
    ...
    function _setTemplate() {
        ...
        $('#id_schedule-start_date').datetimepicker().on('dp.change', function (e) {
            $('#id_schedule-end_date').data("DateTimePicker").minDate(e.date.add(15, 'minutes').toDate());

            let end = $('#id_schedule-end_date').data("DateTimePicker").date();

            if (!end || (end < $('#id_schedule-start_date').data("DateTimePicker").date().utc())) {
                $('#id_schedule-end_date').data("DateTimePicker").date($('#id_schedule-end_date').data("DateTimePicker").minDate().toDate());
            }

            //if (mode === 'basic' || $("form#schedule-form #id_schedule-type").data('location_required') === 'True') {
