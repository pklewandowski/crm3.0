...
class ScheduleCalendarScheduleTypeM2M(models.Model):
    calendar = models.ForeignKey('ScheduleCalendar', on_delete=models.CASCADE)
...
class ScheduleCalendar(models.Model):
    ...
    allowed_event_types = models.ManyToManyField(to='ScheduleType', through=ScheduleCalendarScheduleTypeM2M)
    # view types for calendar:
    # dayGridMonth, businessWeek, timeGridWeek, timeGridDay, listMonth
    allowed_view_types = JSONField(null=True)
    ...
    class Meta:
        ...
        constraints: [
            UniqueConstraint(fields=['name'], name='unique_name_schedule_calendar')
        ]


class Schedule(models.Model):
    SCHEDULE_STATUS_NEW = 'NW'
    SCHEDULE_STATUS_CLOSED = 'CL'

    type = models.ForeignKey('ScheduleType', db_column="id_type", on_delete=models.CASCADE)
    calendar = models.ForeignKey('ScheduleCalendar', db_column="id_calendar", on_delete=models.CASCADE, null=True)
    ...
    title = models.CharField(verbose_name=_('schedule.title'), max_length=200, blank=True)
    ...
    status = models.CharField(verbose_name=_('schedule.status'), max_length=200, default=SCHEDULE_STATUS_NEW, blank=True)
    ...
    def save(self, *args, **kwargs):
        if not self.calendar:
            self.calendar = ScheduleCalendar.objects.get(is_default=True)
...
class ScheduleUser(models.Model):
    ...
    user = models.ForeignKey(User, db_column="id_user", related_name='schedule_user_set', on_delete=models.CASCADE)
    parent = models.IntegerField(null=True, blank=True)
    confirmed = models.BooleanField(default=False)
    participant_type = models.CharField(max_length=1, default='U')
    exclusive_participant_mode = models.BooleanField(default=False)
...
class ScheduleTimeOff(models.Model):
    off_date = models.DateField(verbose_name='schedule.time_off.off_date')
    title = models.CharField(verbose_name='schedule.time_off.title', max_length=300)
    description = models.TextField(verbose_name='schedule.time_off.description', null=True, blank=True)

    def __str__(self):
        return str(self.off_date) + ': ' + str(self.title)
