...
from .models import Schedule, ScheduleType, ScheduleUser, ScheduleNote
...
class ScheduleForm(p3form.ModelForm):
    repeat_period_choices = [('WEEK', 'co tydzień'),
                             # ('2WEEK', 'co drugi tydzień'),
                             # ('MCDAYS', 'co miesiąc w wybrane dni tygodnia'),
                             ('MC', 'wybrane dni miesiąca')]
    ...
    # service = forms.ModelChoiceField(queryset=DocumentType.objects.filter(
    #     category=DocumentTypeCategory.objects.get(code='US')).order_by('name'),
    #                                  empty_label='----', required=False)
    is_cyclical = forms.BooleanField(label=_('schedule.is_cyclical'), required=False)
    repeat_period = forms.ChoiceField(label=_('schedule.repeat_period'), choices=repeat_period_choices, required=False)
    days_of_month = forms.CharField(max_length=200, required=False)
    days_of_week = forms.CharField(max_length=200, required=False)
    participants_json = forms.CharField(max_length=2000, required=False)
    ...
    class Meta:
        model = Schedule
        fields = ['type', 'title', 'description', 'start_date', 'end_date', 'meeting_room', 'participants_json']
...
class ScheduleNoteForm(p3form.ModelForm):
