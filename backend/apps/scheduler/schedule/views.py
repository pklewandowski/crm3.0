...
def _get_participants(participants):
    ...
    users = []
    for i in p:
        users.append({"user": User.objects.get(pk=i['id']), "parent": int(i['parent']) if i['parent'] else None})
    return users
...
def add(request):
    ...
    try:
        ...
        with transaction.atomic():
            ...
            exception_list = []
            invited_users = _get_participants(form.cleaned_data.get('participants_json'))
            ...
            schedule.participant_confirmed = True  # all([i.confirmed for i in invited_users])
            ...
            for i in invited_users:
                ScheduleUser.objects.create(
                    schedule=schedule,
                    user=i['user'],
                    parent=i['parent'],
                    participant_type="P",  # TODO: docelowo pobierać z requestu - dodać funkcjonalność
                    exclusive_participant_mode=True
                )
        ...
        if is_headquarter:
            if headquarter:
                ...
                q = q & (Q(meeting_room__headquarter__in=[i['id'] for i in request.session['user_headquarters']]) |
                         Q(meeting_room__headquarter__isnull=True) |
                         Q(meeting_room__isnull=True)
                         )
    ...
    for i in u:
        l.append(
            {'id': i.user.pk,
             ...
             'text': "%s %s %s" % (i.user.first_name, i.user.last_name, i.user.phone_one if i.user.phone_one else '')
             })
    if len(l):
...
def get_users_for_meeting_filter(request):
    ...
    excluded = json.loads(request.POST.get('excluded'))

    response_data = {}
    status = 200

    try:
        q = Q(last_name__istartswith=key) | Q(phone_one__startswith=key)
        if excluded:
            q = q & ~Q(pk__in=excluded)
