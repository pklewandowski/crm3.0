...
function setHeadquarters() {
    ...
    // $(".fc-left").append(
    //     '<div class="btn-group" role="group">' +
    //     '<button type="button" id="allDayToggleBtn">all day</button>' +
    //     '</div>'
    // );
...
$(document).ready(function () {
    ...
    $('#calendar').fullCalendar({
        customButtons: {
            ...
            //     text: 'Filtruj',
            //     click: function () {
            //         $("#scheduleFilterFormContainer").toggle(200);
            //     }
            // },
            eventRoomToggle: {
                text: "Widok sal",
            ...
            allDayToggle: {
                text: 'allDay',
                click: function () {
                    $('#calendar').fullCalendar('option', 'allDaySlot', !$('#calendar').fullCalendar('option', 'allDaySlot'));
                }
            },
        ...
        header: {
            left: 'prev,next today, filter,eventRoomToggle,allDayToggle',
            center: 'title',
        ...
        viewRender: function () {
            ...
            $(".fc-zoomDefault-button").html('<i class="far fa-dot-circle"></i>');

            $(".fc-vZoomIn-button").html('<i class="fa fa-arrow-down"></i>');
            $(".fc-vZoomOut-button").html('<i class="fa fa-arrow-up"></i>');
            $(".fc-vZoomDefault-button").html('<i class="far fa-dot-circle"></i>');
        },
        eventOrder: function () {
            return false;
        },
        ...
        select: function (start, end) {
            let mEnd = $.fullCalendar.moment(end.format('YYYY-MM-DDTHH:mm'));
            let mStart = $.fullCalendar.moment(start.format('YYYY-MM-DDTHH:mm'));
        ...
        eventResize: function (event, delta, revertFunc) {
            ...
            if (view.name !== 'month') {
                if (mEnd.isAfter(mStart, 'day') && !event.data.whole_day) {
                    $('#calendar').fullCalendar('unselect');
                    revertFunc();
                    swal('Rejestrowanie zdarzeń wielodniowych jest dozwolone jedynie w widoku miesiąca oraz dla zdarzeń wielodniowych.', 'W obecnym widoku można tworzyć zdarzenia jedynie w obrębie jednego dnia', 'warning');
                    return;
                }
            }
            else {
                if (!event.data.whole_day) {

                    $('#calendar').fullCalendar('unselect');
                    swal('Modyfikacja zdarzeń w obecnym widoku jest dozwolona jedynie dla zdarzeń wielodniowych.', '', 'warning');
                    revertFunc();
                    return false;
        ...
        eventDrop: function (event, delta, revertFunc) {
            ...
            console.log(view.name !== 'month' || !event.data.whole_day);
            if (view.name !== 'month') {
                if (mEnd.isAfter(mStart, 'day')) {
                    $('#calendar').fullCalendar('unselect');
                    swal('Rejestrowanie zdarzeń wielodniowych jest dozwolone jedynie w widoku miesiąca oraz dla zdarzeń wielodniowych.', 'W obecnym widoku można tworzyć zdarzenia jedynie w obrębie jednego dnia', 'warning');
                ...
                if (!event.data.whole_day) {
                    $('#calendar').fullCalendar('unselect');
                    swal('Modyfikacja zdarzeń obecnym widoku jest dozwolona jedynie dla zdarzeń wielodniowych.', '', 'warning');
                    revertFunc();
                    return false;
                }
            }
    ...
    $("#allDayToggleBtn").click(function () {
        $('#calendar').fullCalendar('option', 'allDaySlot', !$('#calendar').fullCalendar('option', 'allDaySlot'));
    });
});
