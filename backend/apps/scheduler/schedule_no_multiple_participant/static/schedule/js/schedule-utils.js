...
function eventDataDecode(ev) {
    ...
    console.log(_event.data.whole_day);
...
function getFormContent(mode) {
    ...
    function _setTemplate() {
        if (mode === 'basic') {
            ...
            // if (product_retail_container.length) {
            //     product_retail_container.appendTo('#product_retail_container_basic')
            // }
            // else {
            //     $('#product_retail_container_basic').html($('template#product_retail_template').html())
            // }
        }
        ...
        function setClientList() {
            ...
            $.ajax({
                ...
                dataType: 'json',
                data: {
                    id: $("#id_schedule-employee_").val()
                }
            }).done(function (res) {
                $("#id_schedule-client_").empty();
                ...
                $("#id_schedule-client_").val(null).trigger('change');
            }).fail(function (res) {
                console.log(res.responseJSON.errmsg);
            });
        }

        if (_g.schedule.settings.SCHEDULE_OWNED_CLIENTS_ONLY) {
            setClientList();
        }

        console.log('select2 employee client');

        let allowMultipleUserChoice = true;
        if (mode === 'basic' && allowMultipleUserChoice) {
            console.log('multiple');
            $("form#schedule-form #id_schedule-employee_, form#schedule-form #id_schedule-client_").prop("multiple", "multiple");
        }

        function userSelect2Select(e, item, callback) {
            "use strict";
        ...
        function userSelect2Unselect(evt, el, item, callback) {
            "use strict";
        ...
        function employeeSelect2() {
            ...
            $("#id_schedule-employee_").select2({
                ...
                ajax: {
                    method: 'post',
                    url: _g.schedule.url.getEmployeesForMeetingFilter, //'/schedule/get-employees-for-meeting-filter/', //TODO: DRUT! Wziąć docelowo ze zmiennej globalnej ustqawionej przez {% static... %}
                    dataType: 'json'
                },
                ...
                console.log(e)
                userSelect2Unselect(e, this, $("#id_schedule-client_"), clientSelect2)

            }).on('select2:select', function () {
                userSelect2Select(this, $("#id_schedule-client_"), clientSelect2);
            });
        }

        function clientSelect2() {
            let s;
            if (_g.schedule.settings.SCHEDULE_OWNED_CLIENTS_ONLY) {

                s = $("#id_schedule-client_").select2({
                    theme: 'bootstrap',
                    minimumInputLength: 2,
                    language: "pl",
                    width: '100%'
                ...
                s = $("#id_schedule-client_").select2({
                    ...
                    ajax: {
                        method: 'post',
                        url: _g.schedule.url.getClientsForMeetingFilter, //'/schedule/get-clients-for-meeting-filter/', //TODO: DRUT! Wziąć docelowo ze zmiennej globalnej ustqawionej przez {% static... %}
                        dataType: 'json',
            ...
            s.on('select2:selecting', function (e) {
                ...
                userSelect2Unselect(e, this, $("#id_schedule-employee_"), employeeSelect2)

            }).on('select2:select', function () {
                userSelect2Select(this, $("#id_schedule-employee_"), employeeSelect2);
            });
        }
