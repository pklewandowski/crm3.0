...
class InstalmentSchedule:
    ...
    def calculate(
        ...
        if not self.product:
            if not instalment_number and not instalment_schedule:
        ...
        for idx, i in enumerate(instalment_schedule) if instalment_schedule else enumerate(range(instalment_number)):
            ...
            instalment = self._calculate_instalment(
                ...
                due_date=due_date,
                balloon=idx == len(instalment_schedule) - 1 if instalment_schedule else i == instalment_number - 1
            )
            if not idx:
