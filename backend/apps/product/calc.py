...
class CalculateLoan(Calculation):
    ...
    def update_state(self, dt, dt_str):
        ...
        if dt_str in self.schedule_list:
            ...
            self._capital_required_from_schedule = cap_req # min(cap_req, self.accounting['CAP_NOT_REQ'])
    ...
    def set_calculation_initial_state(self, dt):
        ...
        filter_schedule_list = [i for i in list(
            map(lambda x: datetime.datetime.strptime(x, '%Y-%m-%d').date(), self.schedule_list)) if i < max_date]
    ...
    def calculate(self, start_date=None, end_date=None, simulation=False, emulate_payment=False):
        ...
        if emulate_payment:
            ...
            for k, v in self.schedule_list.items():
                if (datetime.datetime.strptime(k, '%Y-%m-%d').date() >= datetime.date.today()
