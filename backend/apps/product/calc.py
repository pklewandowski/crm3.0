...
class CalculateLoan(Calculation):
    ...
    def __init__(self, product, user):
        ...
        self.schedule_current_date = datetime.datetime.strptime(self.schedule_current_date_str, settings.DATE_FORMAT).date()
    ...
    def __handle_remissions(self, dt_str):
        ...
        if dt_str in self.accounting_list['REM_INTEREST_FOR_DELAY']:
            ...
            val = round(min(self.accounting['INTEREST_FOR_DELAY_VALUE'], val_d), self.decimal_places)
    ...
    def set_calculation_initial_state(self, dt):
        ...
        filter_schedule_list = [i for i in list(
            map(lambda x: datetime.datetime.strptime(x, settings.DATE_FORMAT).date(), self.schedule_list)) if i < max_date]

        try:
            self.schedule_current_date = max(filter_schedule_list)
            self.schedule_current_date_str = datetime.datetime.strftime(self.schedule_current_date, settings.DATE_FORMAT)
