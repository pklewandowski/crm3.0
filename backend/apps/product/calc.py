...
class LoanCalculation(Calculation):
    def __init__(self, product, user):
        ...
        self.accounting['CAPITAL_PAYMENT'] = decimal.Decimal(0.0)
        ...
        self._capital_payment = decimal.Decimal(0.0)
        ...
        self._capital_payment_total = decimal.Decimal(0.0)
    ...
    def calculate_interest_daily(self, dt):
        ...
        if (
                self.interest_code == '1'
                and (
                (self.accounting['COMM_REQ'] > 0 or self.accounting['CAP_REQ'] > 0) and
                due_date is not None and dt >= due_date
        )
        ):
    ...
    def update_state(self, dt, dt_str):
        ...
        if 'CAPITAL_PAYMENT' in self.accounting_list and dt_str in self.accounting_list['CAPITAL_PAYMENT']:
            val = self.accounting_list['CAPITAL_PAYMENT'][dt_str]
            self.accounting['CAPITAL_PAYMENT'] += val
            self._capital_payment = val
            self._capital_payment_total += val
    ...
    def _account_with_order(self, dt_str: str, payment_type: str, ignore_due_day: bool = False):
        for i in self.accounting_order:
            ...
            if dt_str not in self.schedule_list and not ignore_due_day:
                if (i.accounting_type.due_day_accounting_only and
                        datetime.datetime.strptime(dt_str, '%Y-%m-%d') < max(
                            [datetime.datetime.strptime(i, '%Y-%m-%d') for i in self.schedule_list.keys()])
                ):
                    continue
    ...
    def fill_calculation_table(self, dt, dt_str):
        ...
        self._calculation_list.append(
            ProductCalculation(
                ...
                capital_payment=self._capital_payment,
                capital_payment_total=self._capital_payment_total,
    ...
    def _set_calculation_state(self, calc):
        if calc:
            ...
            self._capital_payment = calc.capital_payment
            self._capital_payment_total = calc.capital_payment_total
    ...
    def set_calculation_initial_state(self, dt):
        ...
        # if product statuses are auto-managed.
        ...
        filter_schedule_list = [i for i in list(
            map(lambda x: datetime.datetime.strptime(
                x, settings.DATE_FORMAT).date(), self.schedule_list)) if i < max_date]
    ...
    def calculate(self, start_date=None, end_date=None, simulation=False, emulate_payment=False):
        ...
        with (transaction.atomic()):
            ...
            while dt < self.end_date:
                ...
                self.update_operational_variables(dt_str)
                ...
                self.book_payment(dt_str=dt_str, ignore_due_day=self._undo_interest_for_delay)
