    ...
    def clean(self):
        ...
        for form in self.forms[1:]:
            if form.cleaned_data['launch_date'] and form.cleaned_data['launch_date'] <= dt:
                form.add_error('launch_date', 'Tranche date cannot be older the previous tranche')
                raise forms.ValidationError('Tranche date cannot be older the previous tranche')

            dt = form.cleaned_data['launch_date'] or dt
