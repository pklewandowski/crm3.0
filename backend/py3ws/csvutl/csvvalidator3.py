...
class CSVValidator3(object):
    ...
    def validate_header(self, header_row):
        return self._apply_header_checks(0, header_row)

    def validate_record_length(self, row):
        errors = []
        return self._apply_record_length_checks(0, row)
    ...
    def _apply_unique_checks(self, i, r, unique_sets, summarize=False, context=None):
        ...
        for key, code, message in self._unique_checks:
            ...
            if isinstance(key, str):  # assume key is a field name
                ...
                if fi >= len(r):
                    continue
                # if value is None or '' then not validating. NOT NULL validations are done in VALUE_CHECK_FAILED_NOT_NULL section
                if not r[fi]:
                    continue
    ...
    def _apply_each_methods(self, i, r, summarize=False, report_unexpected_exceptions=True, context=None):
...
""")
        ...
        if not summarize:
            ...
            underline += '\n'
            w(underline)
            for k in sorted(p.viewkeys() - {'code', 'message', 'context'}):
                w(':%s: %s\n' % (k, p[k]))
            if 'context' in p:
                c = p['context']
